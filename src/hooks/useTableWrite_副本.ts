...
export function useTableWrite({
  ...
  const isCellEmpty = (v: any): boolean => {
  ...
  const handleAddMode = async (table: any, items: { f_name: string }[]) => {
    ...
    const selectField = await table.getField(selectFieldId);
    const sourceRecordId = await findSourceRecordId(table, selectField, selectedValue);
    ...
    const values: string[] = multiRecord
      ? items.map((i) => i.f_name)
      : [items.map((i) => i.f_name).join(",")];

    // 以账户列（selectFieldId）是否为空作为"空行"的判定依据
    // 用当前视图的可见顺序获取行，保证从视图中靠前的空行开始填
    ...
    for (const rid of recordIds) {
      if (!rid) continue;
      const v = await selectField.getValue(rid);
      if (isCellEmpty(v)) emptyRecordIds.push(rid);
    }

    let i = 0;
    for (; i < values.length && i < emptyRecordIds.length; i++) {
      const fields = buildBaseFields();
      fields[targetFieldId!] = { type: "text", text: values[i] };
      await table.setRecord(emptyRecordIds[i], { fields });
    }
    const filledCount = i;

    const remaining = values.slice(i);
    let addedCount = 0;
    if (remaining.length > 0) {
      const newRecords: IRecordValue[] = remaining.map((val) => {
        const fields = buildBaseFields();
        fields[targetFieldId!] = { type: "text", text: val };
        return { fields };
      });
    ...
    if (multiRecord) {
      const parts: string[] = [];
      if (filledCount) parts.push(`填充 ${filledCount} 条空白`);
      if (addedCount) parts.push(`新增 ${addedCount} 条`);
      message.success(parts.length ? parts.join("，") : "未执行任何操作");
    } else if (filledCount) {
      message.success(`已填入 1 条空白记录（包含 ${items.length} 个素材）`);
    } else {
      message.success(`已新增 1 条记录（包含 ${items.length} 个素材）`);
    }
  };
